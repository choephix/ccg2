package spatula {
	...
	public class Trail3 extends DisplayObject {
		...
		public function Trail3( numSegments:int = 6, color:uint = 0xffffff ) {
		...
		private function setupVertices():void {
			...
			mVertexData = new VertexData( ( mNumSegments + 1 ) * 3 );
			//mVertexData.setUniformColor( mColor );
			
			for ( i = 0; i <= mNumSegments; ++i ) {
				var c:uint = Math.random() * 0xFFFFFF;
				mVertexData.setAlpha( i * 3 + 0, 0 );
				mVertexData.setColor( i * 3 + 1, c );
				mVertexData.setAlpha( i * 3 + 2, 0 );
			}
			
			mVertexData.setAlpha( 1, 0 );
			mVertexData.setAlpha( mNumSegments*3 + 1, 0 );
			...
			for ( i = 0; i < mNumSegments; ++i ) {
				j = i * 3;
				mIndexData.push( j+0, j+1, j+3 );
				mIndexData.push( j+1, j+4, j+3 );
				mIndexData.push( j+1, j+2, j+4 );
				mIndexData.push( j+2, j+5, j+4 );
			}
		...
		public function arrange( a:Array ):void {
			...
			for ( i = 0; i <= mNumSegments; ++i ) {
				...
				r = 1 - ( i / mNumSegments ) * 1.8;
				x = r * Math.cos( fi ) * mSegmentWidth / 2;
				y = r * Math.sin( fi ) * mSegmentWidth / 2;
				
				mVertexData.setPosition( i * 3 + 0, a[i].x + x, a[i].y + y );
				mVertexData.setPosition( i * 3 + 1, a[i].x + 0, a[i].y + 0 );
				mVertexData.setPosition( i * 3 + 2, a[i].x - x, a[i].y - y );
			}
